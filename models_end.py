...
class NeuralPrecoder(Layer):
    ...
    def build(self, input_shape):
        ...
        self._projection_conv = Conv2D(filters=2*self._num_conv_channels,
                                       kernel_size=(1,1),
                                       padding='same',
                                       activation=None)
        # 4) Four main residual blocks (maintaining 2*self._num_conv_channels).
        self._res_block_1 = ResidualBlock(in_channels=2*self._num_conv_channels,
                                          out_channels=2*self._num_conv_channels,
                                          kernel_size=(7,5),
                                          dilation_rate=(7,1))
        self._res_block_2 = ResidualBlock(in_channels=2*self._num_conv_channels,
...
###############################################################################
# NeuralReceiverPilotChannel
###############################################################################
class NeuralReceiverPilotChannel(Layer):
    r"""
    A residual convolutional neural receiver that takes pilot symbols and 
    channel frequency response (h_freq) as additional inputs.
      - Input: y       [batch, num_rx_ant, num_ofdm_symbols, num_subcarriers], tf.complex
               pilot   [batch, num_rx_ant, num_ofdm_symbols, num_subcarriers], tf.complex
               h_freq  [batch, num_rx_ant, num_ofdm_symbols, num_subcarriers], tf.complex
               no      [batch], tf.float32
      - Same architecture as NeuralReceiverPilot, but input conv sees
        [ real(y), imag(y), real(pilot), imag(pilot), real(h_freq), imag(h_freq), log10(no) ].
      - Output: [batch, num_ofdm_symbols, num_subcarriers, num_bits_per_symbol]
    """
    def __init__(self, num_conv_channels, num_bits_per_symbol, **kwargs):
        super().__init__(**kwargs)
    ...
    def call(self, inputs):
        ...
        # 2) transpose y, pilot to spatial layout
        #    [batch, num_rx_ant, sym, subc] -> [batch, sym, subc, num_rx_ant]
        y      = tf.transpose(y,      [0, 2, 3, 1])
        pilot  = tf.transpose(pilot,  [0, 2, 3, 1])
        
        # h_freq from sionna has shape [batch, num_rx, num_rx_ant, num_tx, num_tx_ant, num_ofdm_symbols, num_subcarriers]
        # We need to reshape it to [batch, sym, subc, num_channels] to match y's layout
        # First flatten all middle dimensions and keep batch, sym, subc
        h_freq_shape = tf.shape(h_freq)
        batch_size = h_freq_shape[0]
        num_ofdm_symbols = tf.shape(y)[1]  # sym dimension after transpose
        num_subcarriers = tf.shape(y)[2]   # subc dimension after transpose
        
        # Reshape to [batch, -1, num_ofdm_symbols, num_subcarriers] then transpose
        # The channel has shape [..., num_ofdm_symbols, num_subcarriers] at the end
        h_freq_flat = tf.reshape(h_freq, [batch_size, -1, num_ofdm_symbols, num_subcarriers])
        # Transpose to [batch, sym, subc, num_channels]
        h_freq = tf.transpose(h_freq_flat, [0, 2, 3, 1])

        # 3) split into real/imag and concat: y, pilot, h_freq, noise
        z = tf.concat([
            tf.math.real(y),
            tf.math.imag(y),
            tf.math.real(pilot),
            ...
            tf.math.imag(h_freq),
            no
        ], axis=-1)  # final channel dim = 6*num_rx_ant + 1
