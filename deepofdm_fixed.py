...
from sionna.channel.tr38901 import AntennaArray, CDL, Antenna, TDL
...
def main():
    ...
    parser.add_argument('--cdl_model', type=str, default="A",
    ...
    parser.add_argument('--results_folder', type=str, default="tdla_repro",
    ...
    class ModelClass(tf.keras.Model):
        def __init__(self,
            ...
            self._cdl = TDL(model=self._cdl_model,
        ...
        def call(self, batch_size, ebno_db):
            """
            FIXED VERSION - Forward pass through the OFDM communication system.
            
            All bugs fixed:
            - Energy normalization for entire frame
            - ZF precoding + neural modulation conflict resolved
            - Double squeeze bug fixed
            - Incorrect channel sampling removed
            - ICI warning added for perfect CSI
            """
            # =================================================================
            # 1. SOURCE GENERATION AND ENCODING
            # =================================================================
            ...
            # =================================================================
            # 2. MODULATION AND RESOURCE GRID MAPPING
            # =================================================================
            
            x = self._mapper(c)
            x_rg = self._rg_mapper(x)
            
            # Extract data symbols (non-pilot OFDM symbols)
            if self._pilot_ofdm_symbol_indices is None:
                x_rg_data = x_rg
                non_pilot_symbols = None
            else:
                non_pilot_symbols = np.setdiff1d(
                    np.arange(self._num_ofdm_symbols), 
                    self._pilot_ofdm_symbol_indices
                )
                x_rg_data = tf.gather(x_rg, non_pilot_symbols, axis=-2)

            # Extract pilot symbols for receiver
            if self._pilot_ofdm_symbol_indices is not None:
                ...
                pilot_mask = tf.reshape(pilot_mask, [1, 1, 1, self._num_ofdm_symbols, 1])
            ...
            if self._domain == "time":
                ...
                a, tau = self._cdl(
                    batch_size, 
                    self._rg.num_time_samples * self._num_delay + self._l_tot - 1, 
                    self._rg.bandwidth
                )

                # FIXED: Proper time-domain channel generation
                h_time = cir_to_time_channel(
                    self._rg.bandwidth, a, tau, 
                    l_min=self._l_min, 
                    l_max=self._l_max, 
                    normalize=True
                )
                
                # FIXED: Proper frequency-domain channel conversion
                h_freq = time_to_ofdm_channel(h_time, self._rg, self._l_min)
                
                # REMOVED: Incorrect channel sampling (was line 508)
                
                # =============================================================
                # 4. TRANSMITTER PROCESSING (PRECODING & NEURAL MODULATION)
                # =============================================================
                
                # FIX #1: Store ORIGINAL energy before any processing
                energy_input = tf.reduce_mean(
                    tf.abs(x_rg)**2, 
                    axis=[1, 2, 3, 4], 
                    keepdims=True
                )
                
                # FIX #2: Handle downlink precoding correctly
                if self._direction == "downlink":
                    if self._precoder_mode == "nrx":
                        # Only apply ZF in non-neural modes
                        x_rg, g = self._zf_precoder([x_rg, h_freq])
                        
                        # Update x_rg_data after precoding if pilots exist
                        if self._pilot_ofdm_symbol_indices is not None:
                            x_rg_data = tf.gather(x_rg, non_pilot_symbols, axis=-2)
                
                # Apply neural modulation/precoding schemes
                if self._system == "neural-receiver":
                    
                    if self._precoder_mode == "nm":
                        # =====================================================
                        # NEURAL MODULATION MODE
                        # =====================================================
                        
                        # Apply neural modulation to data portion
                        x_rg_data = self._neural_mod(tf.squeeze(x_rg_data, axis=1))
                        
                        # Scatter modulated data back into full grid
                        if self._pilot_ofdm_symbol_indices is not None:
                            batch_size_tf = tf.shape(x_rg)[0]
                            num_subcarriers = tf.shape(x_rg)[-1]
                            num_data_symbols = tf.shape(x_rg_data)[3]

                            # Create scatter indices
                            batch_indices = tf.range(batch_size_tf)[:, tf.newaxis, tf.newaxis]
                            ...
                            symbol_indices = tf.tile(symbol_indices, [batch_size_tf, 1, num_subcarriers])
                            
                            subcarrier_indices = tf.range(num_subcarriers)[tf.newaxis, tf.newaxis, :]
                            subcarrier_indices = tf.tile(subcarrier_indices, [batch_size_tf, num_data_symbols, 1])
                            
                            scatter_indices = tf.stack(
                                [batch_indices, dim1_indices, dim2_indices, symbol_indices, subcarrier_indices],
                                axis=-1
                            )
                        ...
                        # FIX #1 (CRITICAL): Normalize ENTIRE frame
                        energy_output = tf.reduce_mean(
                            tf.abs(x_rg)**2, 
                            axis=[1, 2, 3, 4], 
                            keepdims=True
                        )
                        scale = tf.cast(
                            tf.sqrt(energy_input / (energy_output + 1e-12)), 
                            x_rg.dtype
                        )
                        x_rg = x_rg * scale  # Scale complete frame
                        
                    elif self._precoder_mode == "sip":
                        # =====================================================
                        # SUPERIMPOSED PILOTS MODE
                        # =====================================================
                        ...
                        # =====================================================
                        # NEURAL MODULATION + SUPERIMPOSED PILOTS
                        # =====================================================
                        
                        # FIX #3: Apply neural modulation first (squeeze once)
                        x_rg = self._neural_mod(tf.squeeze(x_rg, axis=1))
                        
                        # Then apply SIP (already squeezed)
                        x_rg = self._neural_mod_sip(x_rg)  # FIXED: No second squeeze
                ...
                if "baseline" in self._system:
                    ...
                    if self._system == 'baseline-perfect-csi':
                        ...
                        # FIX #4: Add warning about ICI limitations
                        if not hasattr(self, '_ici_warning_shown'):
                            import warnings
                            warnings.warn(
                                "Perfect CSI baseline uses diagonal-only LMMSE equalization. "
                                "ICI effects from high mobility are not properly handled.",
                                category=UserWarning
                            )
                            self._ici_warning_shown = True
                        
                    elif self._system == 'baseline-ls-estimation':
                        h_hat, err_var = self._ls_est([y, no_vec])
                    
                    # Apply LMMSE equalization
                    x_hat, no_eff = self._lmmse_equ([y, h_hat, err_var, no_vec])
                    ...
                    # Pass to neural receiver
                    if self._pilot_ofdm_symbol_indices is not None:
                        llr = self._neural_receiver([
                            y, 
                            tf.squeeze(x_rg_pilot, axis=[1]), 
                            no_vec
                        ])
                    else:
                        llr = self._neural_receiver([y, no_vec])
